...
export const getAllCartByEmail = async (Email: string, token: string): Promise<Cart> => {
  try {
    const response = await axios.get<Cart>(`http://192.168.1.12:3030/api/cart/getByEmail/`, {
      headers: {
        Authorization: token
      },
      params: {
...
export const deleteById = async (cartId: string, token: string, courseId: string): Promise<Cart> => {
  try {
    const response = await axios.post<Cart>(`http://192.168.1.12:3030/api/cart/deteleCourseOutCart`, {
      cartId: cartId,
      courseId: courseId,
    }, {
      headers: {
